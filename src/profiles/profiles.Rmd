...
f_x_print <- function( poly, precision = 2 ) {
  facts <- lapply(1:length(poly), function(t) { paste( format(poly[t], digits=2, nsmall= precision),"x^", (t-1), sep="")} )
...
for( x in 1:nrow(y_building_info$profiles[[i]]$centers) ) {
  ...
  bp <- barplot(as.table(rbind(bl,ch)), names.arg = list_hours, ylab="kWh", xlab="Hora", sub=sub_t, ylim=c(0,max_load))
  # Carga base Media 
  lines(x=bp,y=rep(y_building_info$base_load[[i]]$dmc_wo_stats$mean,length(bl)), col="blue")
  # Carga base Máx
  lines(x=bp,y=rep(y_building_info$base_load[[i]]$dmc_wo_stats$max,length(bl)), col="red")
  # Aproximación mediante un polinomio
