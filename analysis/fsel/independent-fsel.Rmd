...
set.seed(1)
library(scmamp)
setwd("/home/andu/git/nrgy/analysis/fsel")
```
...
``` {r echo=FALSE, warning=FALSE}
fit_matrix <- matrix( c(fitness_ssga, fitness_rand, fitness_ig, fitness_cfs), ncol=4, dimnames = list(1:64,c("SSGA","RS","IG","CFS")))

# Si p-value es menor al umbral definido, entonces se rechaza la hipótesis nula (todos los algoritmos se comportan igual)
f_a_t <- friedmanAlignedRanksTest(fit_matrix)

#raw_p <- friedmanPost(fit_matrix)
#adjustHolland(raw_p)
#friedmanAlignedRanksPost(fit_matrix)
pv_matrix <- friedmanAlignedRanksPost(fit_matrix, control=1)
pv_adj <- adjustHolland(pv_matrix)
```

El p-value para el test de Friedman Alineado es igual a `r format(f_a_t$p.value, digits=3)`, por lo que se rechaza la hipótesis nula, lo que implica que los algoritmos se comportan de manera distinta.

Ahora comparamos los resultados de cada algoritmo, utilizando como control los valores obtenidos por el SSGA.

| Comparación | p-value |
| :---------- | ------: |
| SSGA vs RS  | `r format(pv_adj[2], digits=3)` |
| SSGA vs IG  | `r format(pv_adj[3], digits=3)` |
| SSGA vs CFS | `r format(pv_adj[4], digits=3)` |
